...
class _HomeTabState extends State<HomeTab> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    homeBloc.add(SelectGenreEvent(genres[0]));
  }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                        ...
                        return GestureDetector(
                          onTap: () {
                            if (selectedGenreIndex != index) {
                              ...
                              context.read<HomeBloc>().add(SelectGenreEvent(genres[index]));
                            }
                          },
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: const Color(0xFFFFB224)),
                            ),
                            child: Text(
                              genres[index],
                              style: TextStyle(
                  ...
                  BlocBuilder<HomeBloc, HomeState>(
                    buildWhen: (previous, current) =>
                    current is HomeSuggestionsLoadingState ||
                        current is HomeSuggestionsSuccessState ||
                        current is HomeSuggestionsErrorState,
                    builder: (context, state) {
                      if (state is HomeSuggestionsLoadingState) {
                        return const SizedBox(
                          height: 220,
                          child: Center(child: CircularProgressIndicator(color: Color(0xFFFFB224))),
                        );
                      } else if (state is HomeSuggestionsSuccessState) {
                        final suggestionMovies = state.movies;
                        if (suggestionMovies.isEmpty) {
                          return const SizedBox(
                            height: 220,
                            child: Center(
                              child: Text('No movies found for this genre', style: TextStyle(color: Colors.white54)),
                            ),
                          );
                        }
                        return SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final imageUrl = suggestionMovies[index].mediumCoverImage;
                              return Container(
                                width: 146,
                                margin: const EdgeInsets.only(right: 12),
                                decoration: BoxDecoration(
